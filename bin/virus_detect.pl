...
foreach my $sample (@ARGV) 
{
	...
	#my $rr_blast_parameters = "-num_threads $thread_num -word_size $rr_blast_word_size -penalty $mis_penalty -gapopen $gap_cost -gapextend $gap_extension -max_target_seqs $rr_hits_returns";
	...
	if ($mapped_num > 0)
	{
		Util::process_cmd("samtools view -bt $reference.fai $sample.sam -o $sample.bam 2> $TEMP_DIR/samtools.log", $debug) unless (-s "$sample.bam");
		Util::process_cmd("samtools sort $sample.bam -o $sample.sorted.bam 2> $TEMP_DIR/samtools.log", $debug) unless (-s "$sample.sorted.bam");
		Util::process_cmd("samtools mpileup -f $reference $sample.sorted.bam -o $sample.pre.pileup 2> $TEMP_DIR/samtools.log", $debug) unless (-s "$sample.pre.pileup");
