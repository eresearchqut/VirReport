...
main: {
	...
	Util::save_file($blast_table, "$sample_dir/$sample.blastn_temp_table1.txt");	# checking files
	...
	{ 
		...
		Util::save_file($known_coverage, "$sample_dir/$sample.known.cov");  # checking files
		...
		Util::save_file($known_identified, "$sample_dir/$sample.known.identified");
		...
		Util::save_file($known_identified, "$sample_dir/$sample.known.identified_with_depth");
		...
		Util::save_file($known_contig_table, "$sample_dir/$sample_base.blastn.txt");
		...
		if ( length($known_identified) > 1 ) { 
			Util::plot_result($known_identified, $known_contig_blast_table, $sample_dir, $sample_base, 'blastn'); 
		} else {
			unlink "$sample_dir/$sample_base.blastn.xls" if -e "$sample_dir/$sample_base.blastn.txt";
		}
	...
	if ( $novel_check && $num_undetermined > 0 )
	{
		...
		Util::save_file($novel_contig_table, "$sample_dir/$sample_base.blastx.txt");
		...
		if ( length($novel_identified) > 1 ) { 
			Util::plot_result($novel_identified, $novel_contig_blast_table, $sample_dir, $sample_base, 'blastx'); 
		} else {
			unlink "$sample_dir/$sample_base.blastx.txt" if -e "$sample_dir/$sample_base.blastx.txt";
		}
...
sub get_contig_mapped_depth
{
	...
	Util::process_cmd("samtools faidx $contig");
	Util::process_cmd("samtools view -bt $contig.fai $sample.sam -o $sample.bam 2>$sample.samtools.log");
	Util::process_cmd("samtools sort $sample.bam -o $sample.sorted.bam 2>$sample.samtools.log");
	Util::process_cmd("samtools mpileup -f $contig $sample.sorted.bam -o $sample.pileup 2>$sample.samtools.log"); 
