...
sub plot_result
{
	my ($known_identified, $known_contig_blast_table, $output, $sample_base, $type) = @_;
	...
	my $output_file = "$output/$sample_base.$type.html";
	my $output_file2 = "$output/$sample_base.$type.summary.txt";	
  ...
  <tr>
	...
	<td width="50%">$desc</td>	
		...
		#$out_table_txt .= qq' $ref\t$len\t$cov_base ($coverage)\t$contig_num\t$raw_depth\t$norm_depth\t$iden\t$max_iden\t$min_iden\t$genus\t$desc';
		$out_table_txt .= "$sample_base\t$ref\t$len\t$cov_base ($coverage)\t$contig_num\t$raw_depth\t$norm_depth\t$iden\t$max_iden\t$min_iden\t$genus\t$desc\n";

		
		#print $out_table_txt $string2;
	}
...
$out_table
	...
	#$out_table_txt = "$ref\t$len\t$cov_base ($coverage)\t$contig_num\t$raw_depth\t$norm_depth\t$iden\t$max_iden\t$min_iden\t$genus";

	# output table to file
	save_file($out_table, $output_file);
	save_file($out_table_txt, $output_file2);
