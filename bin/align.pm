...
sub runVelvet 
{
	...
	Util::process_cmd("velveth $outputDir $kmer_length -$file_type $sample >> $temp_dir/velvet.log", $debug);
	Util::process_cmd("velvetg $outputDir -cov_cutoff $cov_cutoff -min_contig_lgth 30 >> $temp_dir/velvet.log", $debug);	
	return 1;
}
...
sub base_correction
{
	...
	Util::process_cmd("samtools view -bS $read_file.sam -o $read_file.bam 2> $temp_dir/samtools.log");
	Util::process_cmd("samtools sort $read_file.bam -o $read_file.sorted.bam 2> $temp_dir/samtools.log");
	Util::process_cmd("samtools mpileup -f $contig_file $read_file.sorted.bam -o $read_file.pileup 2> $temp_dir/samtools.log");
